...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav className="flex items-center justify-between h-30 px-6 md:px-10">
        {/* LOGO */}
        <Link href="/" className="h-19 z-1001">
          <svg
        ...
        {/* DESKTOP MENU */}
        <div className="hidden md:flex items-center gap-2 bg-white rounded-xl p-2 font-bold">
          <Link className="px-4 py-2 bg-blue-400 rounded-xl" href="#">
            Expertises
          </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`md:hidden z-1001 w-10 h-10 flex items-center justify-center rounded-md ${menuOpen ? "bg-white" : "bg-brand-pink"} `}
          >
            <div className="relative w-6 h-6 flex flex-col justify-between items-center">
              <span
                className={`absolute w-full h-0.5 bg-black transition-all duration-300 ${
                  menuOpen ? "rotate-45 top-1/2" : "top-1"
                }`}
              />
              <span
                className={`absolute w-full h-0.5 bg-black transition-all duration-300 ${
              ...
              <span
                className={`absolute w-full h-0.5 bg-black transition-all duration-300 ${
                  menuOpen ? "-rotate-45 top-1/2" : "bottom-1"
                }`}
              />
            </div>
          </button>

          {/* CTA (hidden when menu open on mobile) */}
          <div className={`${menuOpen ? "hidden" : "block"} md:block   `}>
            <motion.button
              whileHover={{ scale: 1.08, rotate: -6 }}
              transition={{ type: "spring", stiffness: 300, damping: 15 }}
              className="hidden md:flex items-center gap-2 bg-brand-pink px-3 py-2 rounded-xl font-bold "
            >
      ...
      <motion.div
        ...
        className="fixed top-0 left-0 w-full h-screen bg-brand-pink z-1000 flex flex-col justify-between md:hidden"
      >
        {/* CENTER LINKS */}
        <div className="flex flex-col items-center justify-center flex-1 gap-8 text-3xl font-bold">
          ...
          {/* BOTTOM CTA */}
          <div className="p-6">
            <button className="w-full gap-3 flex items-center justify-between text-white bg-black px-4 py-3 rounded-xl font-bold text-lg">
              <span>Get Results</span>
              <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
